...
public:
	...
	void FillTriangle(std::vector<glm::vec4> vertices, glm::vec3 faceColor, glm::vec3 edgesColor);
	void renderFaces(std::vector<Face>& faces, std::vector<glm::vec4>& finalVertices, bool isActiveModel = false, GLfloat scaleNormalLength = 1, bool isShowFaceNormals = false, bool isShowVertexNormals= false, glm::vec3& edgesColor = glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3& facesColor =glm::vec3(0.5f, 0.5f, 0.5f), glm::vec3& normalsColor = glm::vec3(0.8f, 0.8f, 0.8f), std::map<int, glm::vec3>& indexesTovertexNormals = std::map<int, glm::vec3>());
